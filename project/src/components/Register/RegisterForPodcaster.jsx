...
export default function RegisterForPodcaster(props) {
  ...
  const [isLinkNotEmpty, SetisLinkNotEmpty] = useState(false);
  ...
  const handleNicknameChange = (e) => {
    ...
    if (!nicknamePattern.test(value)) {
      setErrorNickName(
        "Никнейм должен начинаться с @ и содержать только английские буквы, длина от 3 до 20 символов",
      );
    } else {
      setErrorNickName("");
    }
  ...
  const handleDescriptionChange = (e) => {
    ...
    if (!descriptionPattern.test(value)) {
      setErrorDes(
        "Описание профиля может содержать только буквы, цифры, пробелы и знаки препинания (до 40 символов)",
      );
  ...
  useEffect(() => {
    ...
    errorPassword,
    isChecked,
  ]); // Зависимости для проверки валидации Мы добавили зависимости в массив, чтобы useEffect вызывался каждый раз, когда изменяются состояния ошибок.
    ...
    <>
      <form onSubmit={handleSubmit}>
        ...
        <div className="_frame_8820">
          ...
          <div className="inputplustext1">
            <div
              className={`  ${errorname ? "_red1" : "_blacknewpod3"}  ${focusedState.input2 && errorname.trim() === "" ? "_pink1" : "_blacknewpod3"} ${isNameNotEmpty && errorname.trim() === "" && focusedState.input2 == false ? "_green1" : "_blacknewpod3"}`}
            >
              <input
                type="name"
            ...
            {errorname && <div className="text_for_mistake">{errorname}</div>}
          </div>
          <div className="inputplustext1">
            <div
              className={`  ${errorNickName ? "_red1" : "_blacknewpod3"}  ${focusedState.input3 && errorNickName.trim() === "" ? "_pink1" : "_blacknewpod3"} ${isNickNameNotEmpty && errorNickName.trim() === "" && focusedState.input3 == false ? "_green1" : "_blacknewpod3"}`}
            >
              <input
                type="text"
          ...
          <div className="inputplustext1">
            <div
              className={`  ${errorLink ? "_red1" : "_blacknewpod3"}  ${focusedState.input5 && errorLink.trim() === "" ? "_pink1" : "_blacknewpod3"} ${isLinkNotEmpty && errorLink.trim() === "" && focusedState.input5 == false ? "_green1" : "_blacknewpod3"}`}
            >
              <input
                type="text"
                className={`_rectangle16 ${errorLink ? "rectangle-error1" : ""}`}
                placeholder="Ссылка на mave"
                value={link}
            ...
            {errorLink && <div className="text_for_mistake">{errorLink}</div>}
          </div>

          <div className="inputplustext1">
            <div
              className={`  ${errorDes ? "_red1" : "_blacknewpod4"}  ${focusedState.input4 && errorDes.trim() === "" ? "_pink1" : "_blacknewpod4"} ${isDesNotEmpty && errorDes.trim() === "" && focusedState.input4 == false ? "_green1" : "_blacknewpod4"}`}
            >
            ...
            {errorDes && <div className="text_for_mistake">{errorDes}</div>}
          </div>

          <div className="frame5_1_desk">
            <img src={add} alt="" className="add_desk" />
            <div className="text_for_download_desk">Загрузить фото профиля</div>
          </div>

          <div className="frame5_2_desk">
            <img src={add} alt="" className="add_desk" />
            <div className="text_for_link_desk">Прикрепить ссылку</div>
          </div>

          <div className="inputplustext1">
            {/* <div className={`_rectangle15_3green ${errorPassword ? 'rectangle-container-error' : ''} ${focusedState.input6 ? '_rectangle15_3pink' : ''}`} > */}
            <div
              className={`  ${errorPassword ? "_red1" : "_blacknewpod3"}  ${focusedState.input6 && errorPassword.trim() === "" ? "_pink1" : "_blacknewpod3"} ${isPassNotEmpty && errorPassword.trim() === "" && focusedState.input4 == false ? "_green1" : "_blacknewpod3"}`}
            >
              <input
                type="password"
          ...
          <div className="btnnForReg-user">
            ...
            <div className="btnnForRegPodcaster1">
              {isButtonDisabled1 === true ? (
                <MainButton2 disabled={isButtonDisabled1}>
                  Создать аккаунт
                </MainButton2>
              ) : isHovered ? (
