...
export default function Modal({ isOpen, onClose }, props) {
  ...
  const handleNameChange = (e) => {
    ...
    if (!descriptionPattern.test(value)) {
      setErrorName(
        "Название поста может содержать только буквы, цифры, пробелы и знаки препинания (до 40 символов)",
      );
  ...
  const handleDesChange = (e) => {
    ...
    if (!descriptionPattern.test(value)) {
      setErrorDes(
        "Описание поста может содержать только буквы, цифры, пробелы и знаки препинания (до 40 символов)",
      );
    } else if (value.length > 40) {
      ...
      <div className="modal-contentNewPod">
        ...
        <form className="formformodal" onSubmit={handleSubmit}>
          <div className="obedinforinput">
            <div
              className={`  ${errorName ? "_red" : "_blacknewpod"}  ${focusedState.input1 && errorName.trim() === "" ? "_pink" : "_blacknewpod"} ${isNameNotEmpty && errorName.trim() === "" ? "_green" : "_blacknewpod"}`}
            >
              <input
                type="text"
            ...
            <div
              className={`  ${errorDes ? "_red" : "_blacknewpod"}  ${focusedState.input2 && errorDes.trim() === "" ? "_pink" : "_blacknewpod"} ${isDesNotEmpty && errorDes.trim() === "" ? "_green" : "_blacknewpod"}`}
            >
              <input
                type="text"
                value={des}
            ...
            <div
              className={`  ${errorTags ? "_red" : "_blacknewpod"}  ${focusedState.input3 && errorTags.trim() === "" ? "_pink" : "_blacknewpod"} ${isTagsNotEmpty && errorTags.trim() === "" ? "_green" : "_blacknewpod"}`}
            >
              <input
                type="text"
            ...
            {errorTags && <div className="text_for_mistake">{errorTags}</div>}

            <div
              className={`  ${errorAge ? "_red" : "_blacknewpod"}  ${focusedState.input4 && errorAge.trim() === "" ? "_pink" : "_blacknewpod"} ${isAgeNotEmpty && errorAge.trim() === "" ? "_green" : "_blacknewpod"}`}
            >
              <input
                type="text"
            ...
            <div className="frame5_1">
              ...
              <div className="text_for_download">Обложка</div>
            </div>
