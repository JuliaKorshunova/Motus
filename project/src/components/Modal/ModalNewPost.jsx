...
export default function ModalNewPost({ isOpen, onClose }, props) {
  ...
  const handlePostChange = (e) => {
    ...
    if (!descriptionPattern.test(value)) {
      setErrorPost(
        "Текст поста может содержать только буквы, цифры, пробелы и знаки препинания (до 40 символов)",
      );
      ...
      <div className="modal-contentNP">
        ...
        <form className="formformodal" onSubmit={handleSubmit}>
          <div className="obedinforinput">
            <div
              ref={textareaRef}
              className={`_bk ${focusedState.input6 && errorPost.trim() === "" ? "_pink" : ""} ${isTextareaNotEmpty && errorPost.trim() === "" ? "_green" : ""} ${errorPost ? "_red" : ""} `}
            >
              <textarea
                ref={textareaRef}
            ...
            {errorPost && <div className="text_for_mistake">{errorPost}</div>}

            <div className="frame5_1">
              <img src={add} alt="" className="add" />
              <div className="text_for_download">Прикрепить фото</div>
            </div>
